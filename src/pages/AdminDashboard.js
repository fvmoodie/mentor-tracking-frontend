...
const AdminDashboard = () => {
  ...
  const handleMenuClick = (menu) => {
    ...
    if (menu === "Assign region and roles") {
      navigate("/assign-regionrole");
    }
    if (menu === "Create regions") {
      navigate("/regions-management");
    }
  };
    ...
    <div className="container">
      ...
      <div className="top-row">
        <div className="top-left">
          ...
          <span className="welcome-text">System Management</span>
        </div>
      ...
      <div className="middle-row">
        {/* Column 1: Admin Features with Submenus */}
        <div className="column features-column">
          {[
            { title: "👥 User Management", subItems: ["Create regions", "Assign region and roles","Assign roles", "Manage permissions"] },
            ...
            { title: "🔒 Security & Compliance", subItems: ["Monitor security logs", "Manage authentication settings", "Audit logs"] },
              ...
              {activeMenu === menu.title && (
                <div className="sub-menu">
                  {menu.subItems.map((item, i) => (
                    <p key={i} className="sub-menu-item" onClick={() => handleMenuClick(item)}>{item}</p>
                  ))}
                </div>
              )}
            </div>
